...
// LEADER: arrowhead point, one or more landing points (Enter to stop picking),
// then annotation lines (empty line finishes). Creates a LeaderEntity plus an
// MTEXT at the landing, sized from the document's dimension style, as one
// undo step. Finishing with no text leaves just the leader line.
class LeaderCommand : public DrawCommand {
public:
    explicit LeaderCommand(lcad::Document& document) : m_document(document) {}
