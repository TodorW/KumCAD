...
BoundingBox WireEntity::boundingBox() const {
    BoundingBox box;
    for (const Point2D& v : m_vertices) box.expand(v);
    return box;
}

double WireEntity::distanceTo(const Point2D& pt) const {
...
void WireEntity::translate(const Point2D& delta) {
    for (Point2D& v : m_vertices) v = v + delta;
}

void WireEntity::rotate(const Point2D& center, double angleRadians) {
    for (Point2D& v : m_vertices) v = rotateAround(v, center, angleRadians);
}

void WireEntity::scale(const Point2D& center, double factor) {
    for (Point2D& v : m_vertices) v = scaleAround(v, center, factor);
}

void WireEntity::mirror(const Point2D& a, const Point2D& b) {
    for (Point2D& v : m_vertices) v = mirrorAcross(v, a, b);
}

std::vector<Point2D> WireEntity::gripPoints() const { return m_vertices; }

void WireEntity::moveGripPoint(std::size_t index, const Point2D& newPos) {
    if (index < m_vertices.size()) m_vertices[index] = newPos;
}

std::vector<SnapPoint> WireEntity::snapCandidates() const {
...
std::unique_ptr<Entity> WireEntity::clone() const { return std::make_unique<WireEntity>(*this); }
